...
Since some columns have names that may be difficult to interpret without looking at the metadata first, we should rename them.
...
Also, we'll choose another group of clients which have at least one loan with the bank and are at least of the age of 24.

```{r}
withLoans <- bank_full %>% 
  filter((housing_loan == TRUE | personal_loan == TRUE) & age >= 24)
```
...
cleanedFrames <- lapply(bothFrames, function(df) {
  df %>% select(-all_of(c("previous", "poutcome")))
})
```

We may also look at the summarizing statistics.
